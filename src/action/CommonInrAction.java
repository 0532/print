...
@ManagedBean
@ViewScoped
public class CommonInrAction {

    private static Logger logger = Logger.getLogger(CommonInrAction.class);
    ...
    public void init() {
        ...
        invNOtmp = commonInrService.getMaxInvNo();
    ...
    public void getBIData() {
        ...
        try {
            invIntDataList = commonInrService.getBIData(exportDataDate);
        } catch (Exception e) {
    ...
    public void importToDB() {
        int recordNum;
        try {
            recordNum = commonInrService.importToDB();
        } catch (Exception e) {
            addMessage(FacesMessage.SEVERITY_ERROR, "数据库连接或SQL执行出现错误!");
            logger.error(new Date().toString() + " 数据库连接或SQL执行出现错误!");
            return;
    ...
    public void onQueryInvInfo() {
        try {
            invInvInfoList = commonInrService.onQueryInvInfo(invInfoQryCond);
        } catch (Exception e) {
            addMessage(FacesMessage.SEVERITY_ERROR, "数据库连接或SQL执行出现错误!");
            logger.error(new Date().toString() + " 数据库连接或SQL执行出现错误!");
        }
    ...
    public void invalidInvInfo(String invType) {
        try {
            if (commonInrService.invalidInvInfo(invType)) {
    ...
    public List<StaticInvItem> qryExpData() throws Exception {
        return commonInrService.qryExpData(invInfoQryCond);
    }
    ...
    public void onQueryInvData() {
        try {
            invIntDataList = commonInrService.onQueryInvData(invIntDataQryCond);
        } catch (Exception e) {
            addMessage(FacesMessage.SEVERITY_ERROR, "数据库连接或SQL执行出现错误!");
            logger.error(new Date().toString() + " 数据库连接或SQL执行出现错误!");
        }
    ...
    public BigDecimal getCurRat() throws Exception {
        ...
        if (invIntDataQryCond.getCurrencyType().equals("CNY")) {
            ...
            String ratdat = invIntDataQryCond.getTxndat();
            String curratDate = ratdat;

            if (invIntDataQryCond.getCurrencyType().equals("HKD")) {
                currat = commonInrService.getCurRat(curratDate, "013");
            } else if (invIntDataQryCond.getCurrencyType().equals("USD")) {
                currat = commonInrService.getCurRat(curratDate, "014");
            } else {
                currat = commonInrService.getCurRat(curratDate, "087");
            }
        }
        return currat;
    }
    ...
    public void invPreview() {
        ...
        if (!"".equals(invIntDataQryCond.getTxndat())) {
            ...
            List<InvItem> invItems = commonInrService.staticInvItems(invIntDataQryCond, curRat);
            if (!invItems.isEmpty()) {
                fillDatas(invItems);
            }
        } else {
    ...
    public void updateData() {
        ...
        if (invIntDataList != null) {
            try {
                invIntDataList = commonInrService.onQueryInvData(invIntDataQryCondTmp);
            } catch (Exception e) {
                addMessage(FacesMessage.SEVERITY_ERROR, "数据库连接或SQL执行出现错误!");
                logger.error(new Date().toString() + " 数据库连接或SQL执行出现错误!");
            }
    ...
    public void printThread(InvInvInfo invInvInfo) {
        try {
            ...
            commonInrService.addInvInvInfo(invInvInfo);
            commonInrService.addIntItems(invItemsTmp);
            commonInrService.invalidInvIntData(invIntDataQryCondTmp, invItemsTmp);
            try {
                Thread.sleep(2000);
    ...
    public void initCurrentData() {
        try {
            currentInvPriceSum = commonInrService.getCurInvPriceSum();
    ...
    public void invalidInvIntData() {
        try {
            if (selectedInvIntData.getIntAmt().compareTo(new BigDecimal(10000000)) > 0) {
                commonInrService.invalidInvIntData(selectedInvIntData);
    ...
    public void showInvItem() {
        if (selectedInvInvInfo != null) {
            try {
                invItemList = commonInrService.getInvItemsByInvNo(selectedInvInvInfo.getInvCode(), selectedInvInvInfo.getInvNo());
            } catch (Exception e) {
                logger.error(new Date().toString() + " 数据库连接或SQL执行出现错误!");
                addMessage(FacesMessage.SEVERITY_ERROR, "数据库连接或SQL执行出现错误!");
            }
    ...
    public void printBackInv() {
        ...
        boolean invInfoExist = commonInrService.getInvInfoByFadeInvNoAndCode(selectedInvInvInfo.getInvCode(), selectedInvInvInfo.getInvNo());
        if (invInfoExist) {
        ...
        try {
            BigDecimal curBIPS = commonInrService.getCurBackInvPriceSum();
            if (curBIPS != null) {
            ...
            //获取发票明细列表
            List<InvItem> backInvItems = commonInrService.getInvItemsByInvNo(selectedInvInvInfo.getInvCode(), selectedInvInvInfo.getInvNo());
    ...
    public void prntBackInvThread(List<InvItem> backInvItems) throws IOException {
        ...
        try {
            commonInrService.addInvInvInfo(selectedInvInvInfo);
            commonInrService.addIntItems(backInvItems);
            commonInrService.validInvIntData(invCode, invNo);
        } catch (Exception e) {
            logger.error(new Date().toString() + " 数据库连接或SQL执行出现错误!");
            addMessage(FacesMessage.SEVERITY_ERROR, "数据库连接或SQL执行出现错误!");
        }
